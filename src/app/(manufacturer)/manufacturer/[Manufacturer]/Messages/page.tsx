...
export default function Messaging() {
    ...
    const reachChat = async () => {
        ...
        let senderMessages = await axios.post(`${backend_url}/directs/manufacturer-chat/${idStorage}/`, 
            { "id": parseInt(params.Manufacturer)}
        )
        let responseSender = senderMessages.data

        let receiverMessages = await axios.post(`${backend_url}/directs/client-chat/${params.Manufacturer}/`, 
            { "id": id}
        )
    ...
    const getUnread = async () => {
        let request = await axios.post(`${backend_url}/directs/manufacturer-unread/`, 
            { "id": parseInt(params.Manufacturer)}
        )
        ...
        for(let item of response){
            ...
            let clientRequest = await axios.get(`${backend_url}/client/client-detail/${item.sender}`)
            let manFirst = clientRequest.data.first_name
            let manLast = clientRequest.data.last_name
            let itemChat: chat = {
                body: item.body,
    ...
    const getRead = async () => {
        let request = await axios.post(`${backend_url}/directs/manufacturer-inbox/`, 
            { "id": parseInt(params.Manufacturer)}
        )
        ...
        for(let item of response){
            ...
            let clientRequest = await axios.get(`${backend_url}/client/client-detail/${item.sender}`)
            let clientFirst = clientRequest.data.first_name
            let clientLast = clientRequest.data.last_name
            let itemChat: chat = {
                body: item.body,
                client_user:item.client_user,
                created: item.created,
                id: item.id,
                read: item.read,
                senderFirst: clientFirst,
                senderLast: clientLast,
                senderId: item.sender
            }
            chats.push(itemChat)
        }
    ...
    const getChat = async () => {
        ...
        let senderMessages = await axios.post(`${backend_url}/directs/manufacturer-chat/${currentChat?.id}/`, 
            { "id": parseInt(params.Manufacturer)}
        )
        let responseSender = senderMessages.data
    ...
    const sendMessage = async () => {
        let client = currentChat?.id
        let request = await axios.post(`${backend_url}/directs/manufacturer-send/`, 
            { "manufacturer_id": parseInt(params.Manufacturer), "client": client, "body": text}
        )
