...
import { manufacturer, rating } from "@/components/offercard";
import OfferCard from "@/components/offercard";
import Link from "next/link";
import Messaging from "../../Messages/page";
...
export default function CReqDetail() {
    let router = useRouter()
    let params = useParams<{Client : string; Detail: string}>()
    ...
    let [ manufacturerId, setManufacturerId ] = useState<number>(0)
    let [ manufacturerName, setManufacturerName ] = useState<string>('')
    ...
    const GetRequest = async () => {
        ...
        const GetClient = async () => {
            const response = await axios.get(`${backend_url}/client/client-detail/${params.Client}/`)
    ...
    const handleMessage = () => {
        localStorage.setItem('chatId', `${manufacturerId}`);
        localStorage.setItem('ChatName', `${manufacturerName}`);
        router.push(`/client/${params.Client}/Messages`)
        ...
        return (
            ...
            >
                <div style={{textAlign: 'center', marginTop:'60px', width:'80%'}}>
                    ...
                    <div className='flex'>
                        ...
                        filePreviews.map((preview, index) => (
                            <div className="mt-2 mb-8 mx-auto" key={index}>
                                ...
                                <Link href={`${backend_url}${preview}`} target="_blank" rel="noopener noreferrer">view</Link>
                            </div>
                        ))
                        :
                        <p></p>
                    ...
                    {/* <button className="bg-red-600 rounded-lg px-24 py-2 mt-4">Send an offer</button> */}
                    { offerManufacturers && (
                        <div>
